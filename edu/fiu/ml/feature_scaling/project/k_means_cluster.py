...
def cluster2FeaturesWithScaling():
    ...
    for key in data_dict.keys():
        if not data_dict[key]['exercised_stock_options']=="NaN":
            #print "stock:",float(data_dict[key]['exercised_stock_options'])
            scaledStock = float(float(data_dict[key]['exercised_stock_options']) - float(minStock))/float(float(maxStock) - float(minStock))
            #check if the original stock was 1M and print the scaled one
            if float(data_dict[key]['exercised_stock_options'])==1000000.0:
                print data_dict[key]['exercised_stock_options'], scaledStock
            data_dict[key]['exercised_stock_options'] = "%.3f" % scaledStock
            #print data_dict[key]['exercised_stock_options']
        if not data_dict[key]['salary']=="NaN":
            print "salary:",float(data_dict[key]['salary'])
            scaledSal = float(float(data_dict[key]['salary']) - float(minSal))/float(float(maxSal) - float(minSal))
            #check if the original sal was 200,000 and print the scaled one
            if float(data_dict[key]['salary'])==200000.0:
                print data_dict[key]['salary'], scaledSal
...
#do minmaxscaling of given values
...
if __name__=="__main__":
    ...
    scaledSal, scaledStock = rescaleSalAndStockValues()
    print "scaled sal:",scaledSal
    print "scaled stock:",scaledStock
